...
db             = datapump_utils.Database.easy_connect(env=env,datastore='ETLSTAGING',autocommit=True)
...
while True:
	...
	for job in jobs:
		...
		db.execute(f'update etl_forecast set is_started=true where etl_forecast_id={etl_forecast_id}')
		process=getattr(datapump_etl_entry,job['entrypoint'])
		print('launching job {job_name}'.format(job_name=job['job_name']))
		thread=threading.Thread(target=process,args=[job['job_name']])
		thread.start()
	...
	#dev testing
	if loop==30  or glob.glob(processing_dir+'/datapump/'+env+'/maestro/datapump_stop*'):
		break
