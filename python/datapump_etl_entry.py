...
Prerequisistes
	Environment variables
		...
		PROCESSINGDIR: path to ~/_processing. I suggest for dev/testing you create your own local structure example:c:/repos/datapump/_processing. can also use the network drive
		...
		samples
			ENV:dev
			PROCESSINGDIR:c:/repos/datapump/_processing  (this needs to be your local directory)
			PGUSER:nklaskala
			PGPASSWORD:gtfoh
...
def inbound_single_file(job_name,debug=False):
	...
	log_id                       = maestro.etl_log_upsert(job_id=job_id,status='running')
	logger                       = datapump_utils.MyLogger(log_dir=datapump_dir+'/logging',base_name=job_name,log_id=log_id,maestro=maestro)

	#get external file
	external_file_source_path    = (glob.glob(etl_config['source_loc']+'/'+etl_config['file_pattern']))

	if len(external_file_source_path)==0:
		logger.warning('no files to process in: '+etl_config['source_loc']+'/'+etl_config['file_pattern'])
		maestro.etl_log_upsert(log_id=log_id,status='done',end_date=True)
		return 
	...
	for step in etl_config['processes'].split(','):
		...
		try:
			...
			maestro.etl_log_upsert(log_id=log_id,status='done',end_date=True)
			shutil.move(internal_file_staged_path,internal_file_errored_path)
			raise

	#archive file internally
	shutil.move(internal_file_staged_path,internal_file_processed_path)
...
def outbound_single_file(job_name,debug=False):
	...
	log_id                       = maestro.etl_log_upsert(job_id=job_id,file_name=internal_file_name,status='running')
	logger                       = datapump_utils.MyLogger(log_dir=datapump_dir+'/logging',base_name=job_name,log_id=log_id,maestro=maestro)
	...
	shutil.move(internal_file_staged_path,internal_file_processed_path)
	if not debug:
		shutil.copy(internal_file_processed_path,external_file_dest_path)
...
def maintenance(job_name,debug=False):
	...
	maestro.etl_log_reset(job_id=job_id)
	log_id                       = maestro.etl_log_upsert(job_id=job_id,file_name=None,status='running')
	logger                       = datapump_utils.MyLogger(log_dir=datapump_dir+'/logging',base_name=job_name,log_id=log_id,maestro=maestro)
...
if __name__=='__main__':
	...
	argparser.add_argument("-b",  "--debug" ,help="",required=False,default='True')
