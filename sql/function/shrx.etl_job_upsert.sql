drop function if exists shrx.etl_job_upsert;
create or replace function shrx.etl_job_upsert(
	   v_job_name         text 
...
$$
begin
	IF EXISTS (SELECT * FROM shrx.etl_job WHERE job_name = v_job_name)
	then 
		update shrx.etl_job set
			...
			,sumologic      =coalesce(v_sumologic      ,etl_job.sumologic      )
			,config_ext     =coalesce(v_config_ext     ,etl_job.config_ext     )
		where job_name = v_job_name;
	else 
		INSERT INTO shrx.etl_job (job_name,is_active,partner,job_type,source_loc,destination_loc,file_pattern,entrypoint,processes,notes,sumologic,config_ext)
